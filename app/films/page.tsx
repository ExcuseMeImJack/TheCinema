            ...
            <div className='tinted-div'>
              ...
              <div className="text-overlay flex flex-col gap-3 justify-evenly">
                <div>
                  ...
                  <p className='font-bold text-gray-400'>{film.release_date.split('-')[0]}</p>
                </div>
                <p>{film.overview.length > 100 ? (`${film.overview.slice(0, 100)}...`) : film.overview}</p>
                <p className='font-bold text-[var(--highlight)]'>{getRatingStatus(film)}</p>
              </div>
            </div>
          </div>
        ))}
